...
read -r -d '' HANDLER_FISH <<EOF || true
...
inject_rc() {
    ...
    if grep -Eq "command_not_found_hand|fish_command_not_found" "$rc_file" 2>/dev/null; then
...
if [[ "$(basename "$SHELL")" == "zsh" && -f "$HOME/.zshrc" ]]; then
    inject_rc "$HOME/.zshrc" "$HANDLER_ZSH"
fi

if [[ "$(basename "$SHELL")" == "bash" && -f "$HOME/.bash" ]]; then
    inject_rc "$HOME/.bashrc" "$HANDLER_BAH"
fi

if [[ "$(basename "$SHELL")" == "fish" && -f "$HOME/.config/fish/config.fish" ]]; then
    inject_rc "$HOME/.config/fish/config.fish" "$HANDLER_FISH"
fi
